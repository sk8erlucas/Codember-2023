const rawCsv = `PYmt8hd,jdcrLI,jdcrli@gmail.com,27,London
...
const ids = rawCsv.split("\n").map((line) => line.split(",")[0]);
const userNames = rawCsv.split("\n").map((line) => line.split(",")[1]);
const emails = rawCsv.split("\n").map((line) => line.split(",")[2]);
const ages = rawCsv.split("\n").map((line) => line.split(",")[3]);
const locations = rawCsv.split("\n").map((line) => line.split(",")[4]);

let invalidRowsIndexs = [];

for (let i = 0; i < ids.length; i++) {
  // check if the id exists and if is alphanumeric
  if (!ids[i] || !/^[a-zA-Z0-9]+$/.test(ids[i])) {
    invalidRowsIndexs.push(i);
    continue;
  }

  // check if the username exists and if is alphanumeric
  if (!userNames[i] || !/^[a-zA-Z0-9]+$/.test(userNames[i])) {
    invalidRowsIndexs.push(i);
    continue;
  }

  // check if the email exists and if is valid
  if (
    !emails[i] ||
    !/^[a-zA-Z0-9]+@[a-zA-Z0-9]+\.[a-zA-Z0-9]+$/.test(emails[i])
  ) {
    invalidRowsIndexs.push(i);
    continue;
  }

  // check if the age is a number, but is optional
  if (ages[i] && !/^[0-9]+$/.test(ages[i])) {
    invalidRowsIndexs.push(i);
    continue;
  }

  // check if the location is a string, but is optional
  if (locations[i] && !/^[a-zA-Z]+$/.test(locations[i])) {
    invalidRowsIndexs.push(i);
    continue;
  }
}
...
// The invalid rows
console.log("Invalid rows indexs", invalidRowsIndexs);

let secretPassword = "";
invalidRowsIndexs.forEach((index) => {
    // Show the invalid users
    console.log("Invalid user", userNames[index]);
    // Get the first letter of the username
    secretPassword += userNames[index][0];
})
