...
const inValidPasswords = passwordsArray.filter((password) => {
    const [rule, letter, passwordString] = password.split(" ");
    const [min, max] = rule.split("-");
    
    const lettersInPassword = passwordString.split("").filter((char) => char === letter[0]).length;

    if (lettersInPassword < min || lettersInPassword > max) {
        return true;
    }
